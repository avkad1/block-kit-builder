...
exports.validateConversationsSelectInput = (placeholder, actionId, filter) => {
  ...
  if (filter) {
    ...
    assert(
      ...
      "Expected at least one filter to be present"
    );
    filter.include.forEach((filter, index) => {
      assert(
...
exports.validateInput = ({ blockId, hint, element, label }) => {
  ...
  validateText({
    ...
    limit: 2000,
    optional: true,
    message: getCharacterLimitErrorMessage({ field: "hint", limit: 2000, item: hint }),
  });
  ...
  validateText({
    ...
    limit: 2000,
    optional: true,
    message: getCharacterLimitErrorMessage({ field: "label", limit: 2000, item: label }),
  });
};
