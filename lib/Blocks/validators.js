...
const { validateBlockId, validateText, getCharacterLimitErrorMessage } = require("../GenericValidators");
...
exports.imageValidator = ({ url, alt, title, blockId }) => {
  validateText({
    text: url,
    message: getCharacterLimitErrorMessage({ field: "url", limit: 3000, item: url })
  });
  validateText({
    text: alt,
    limit: 2000,
    message: getCharacterLimitErrorMessage({ field: "alt", limit: 2000, item: alt })
  });
  if (title) {
    validateText({
      text: title,
      limit: 2000,
      message: getCharacterLimitErrorMessage({ field: "title", limit: 2000, item: title })
    });
  }
  validateBlockId(blockId);
};
...
exports.sectionFieldsValidator = (fields, blockId) => {
  assert(Array.isArray(fields), `Expected fields to be an array. Received ${typeof fields}`);
  assert(fields.length <= 10, `Cannot have more than 10 fields in a section block. Current fields size: ${fields.length}`);
  fields.forEach((field, index) => {
    validateText({ text: field, limit: 2000, message: getCharacterLimitErrorMessage({ field: "field", limit: 2000, item: field, position: index }) });
  });
  validateBlockId(blockId);
};
