class Blocks {
  ...
  context({ text, blockId }) {
    ...
    /**
   * Context block for accepting array of text and images 
   * @param {Array<Object>} elements elements having text and images block
   * @param {String} [blockId] optional block id
   * @return {{type: string}}
   */
  contextSeries({ elements = [], blockId}) {
    const element = { type: "context", elements: [] };
    if (blockId) {
      element.block_id = blockId;
    }
