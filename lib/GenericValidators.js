...
exports.validateBlockId = (blockId) => {
  if (blockId) {
    ...
    assert(
      blockId.length <= 255,
      this.getCharacterLimitErrorMessage({ field: "blockId", limit: 255, item: blockId })
    );
  }
};
...
exports.optionsValidator = (options, limit = 100) => {
  ...
  assert(
    ...
    this.getCharacterLimitErrorMessage({ field: "options", limit, item: options })
  );
  options.forEach((option, index) => {
    ...
    this.validateText({ text: option.text, limit: 75, message: getCharacterLimitErrorMessage({ field: "option.text", limit: 75, item: option.text }), position: index });
    this.validateText({ text: option.value, limit: 75, message: getCharacterLimitErrorMessage({ field: "option.value", limit: 75, item: option.value }), position: index });
    this.validateText({
      text: option.description,
      limit: 75,
      position: index,
      optional: true,
      message: this.getCharacterLimitErrorMessage({ field: "description", limit: 75, item: option.description })
    });
    this.validateText({
      text: option.url,
      limit: 3000,
      position: index,
      optional: true,
      message: this.getCharacterLimitErrorMessage({ field: "url", limit: 3000, item: option.url })
    });
  });
};
...
exports.validateButton = ({ text, url, actionId, value, dialog = {} }) => {
  ...
  this.validateText({
    ...
    limit: 3000,
    optional: true,
    message: this.getCharacterLimitErrorMessage({ field: "url", limit: 3000, item: url })
  });
  ...
  if (typeof value === "string") {
    this.validateText({
      ...
      limit: 2000,
      optional: true,
      message: this.getCharacterLimitErrorMessage({ field: "value", limit: 2000, item: value })
    });
    ...
    this.validateText({
      ...
      limit: 2000,
      optional: true,
      message: this.getCharacterLimitErrorMessage({ field: "value", limit: 2000, item: JSON.stringify(value) })
    });
  }
  ...
  if (Object.keys(dialog).length) {
    ...
    this.validateText({
      text: title,
      limit: 100,
      message: this.getCharacterLimitErrorMessage({ field: "confirmation title", limit: 100, item: title })
    });
    this.validateText({
      text: description,
      limit: 300,
    ...
    this.validateText({
      text: confirmText,
      limit: 30,
      message: this.getCharacterLimitErrorMessage({ field: "confirmation confirmText", limit: 30, item: confirmText })
    });
    this.validateText({
      text: cancelText,
      limit: 30,
      message: this.getCharacterLimitErrorMessage({ field: "confirmation cancelText", limit: 30, item: cancelText })
    });
  }
};
...
exports.validateUsersSelect = (placeholder, actionId) => {
  ...
  validateText({
    ...
    limit: 150,
    optional: true,
    message: this.getCharacterLimitErrorMessage({ field: "placeholder", limit: 150, item: placeholder })
  });
